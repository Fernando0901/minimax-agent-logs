...
async def execute_via_orchestrator(task_file: Path) -> tuple[bool, str, str]:
    ...
    try:
        ...
        return True, f"Pipeline session: {session_dir}", ""
    except Exception as e:
        import traceback
...
async def process_task(task_file: Path):
    ...
    if success:
        ...
        await send_telegram(f"✅ <b>Tarea completada:</b> {_escape_html(first_line)}\n\n<pre>{preview}</pre>")
    else:
        preview = _escape_html(stderr[:400].strip()) if stderr.strip() else "(sin stderr)"
        await send_telegram(f"❌ <b>Tarea fallida:</b> {_escape_html(first_line)}\n\n<pre>{preview}</pre>")
